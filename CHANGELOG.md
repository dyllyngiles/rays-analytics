...
### 2026-09-04 — `ci.yml` rewrite: DuckDB fully removed, inline key-pair credentials, PR job scoped to Snowflake DEV (PR #39)

**Summary: `ci.yml` is now Snowflake-only in both jobs**, closing out the "CI's post-DuckDB shape" design noted as decided-but-not-implemented in the prior session. Landed on `feature/github-actions-orchestration`, merged via PR #39, six commits:

**1. Bug fix + credential-injection rewrite.** The merge job's "Write Snowflake private key" step was missing the `|` block scalar indicator on its `run:`, so the `chmod 600` line silently folded into the same shell line as the `echo` and never ran as its own command. Rather than patch that in place, replaced the whole tempfile/`chmod`/`private_key_path` pattern in both jobs: `private_key` is now passed inline in the generated `profiles.yml` straight from the `SNOWFLAKE_PRIVATE_KEY` secret — no key file ever written to `$RUNNER_TEMP` or anywhere else on the runner's disk. PR job (`dbt-build`) dropped the "Load MLB data into DuckDB" step and DuckDB `profiles.yml` generation entirely — it never called the ingestion pipeline, only dbt — and now targets Snowflake `DEV` with the same SYSADMIN-tier service-user credentials the merge job already used. Merge job kept its shape (full build, `PROD` schema) with only the credential fix applied.

**2. `uv audit` findings, chased down rather than routed around.** The PR job immediately failed on `uv audit`, surfacing two vulnerabilities:
- `sqlparse 0.5.5` — before touching anything, verified this wasn't a config bug: pulled the advisory text for all four already-ignored GHSA IDs (`GHSA-3496-9g83-7v6x`, `GHSA-f2ff-p2ww-7p4p`, `GHSA-pwgv-4x5q-6m9f`, `GHSA-prg7-hcfm-mfcr`) and confirmed all four are genuinely sqlparse advisories (CVE-2026-59894 code injection, a `group_comments` O(n²) DoS, CVE-2026-54284 parse-tree DoS, CVE-2026-59893 ReDoS), all fixed in 0.6.0. The failing finding, `GHSA-cfqr-cjx5-5jcm` (reindentation-of-tuple-lists near-quadratic CPU DoS, also fixed in 0.6.0), is simply a fifth sqlparse CVE disclosed after the original suppression list was set up — not a different package hiding under the sqlparse comment. Added it to `[tool.uv.audit] ignore` in `pyproject.toml` alongside the other four.
- `snowflake-connector-python 4.6.0` — looked up `GHSA-5cc2-282f-jjq2`/CVE-2026-15925 before bumping: improper TLS hostname verification, exploitable only by an on-path attacker (ARP/DNS poisoning, rogue AP, BGP hijack, malicious proxy) presenting a CA-signed cert for an arbitrary domain, exposing credentials/query data/staged files in transit. Fixed in 4.7.1+. Ran a narrow `uv lock --upgrade-package snowflake-connector-python` (→ 4.7.3) rather than a broad `uv lock --upgrade`, and confirmed via `git diff uv.lock` that no other package moved before committing.

**3. Missing `dbt deps`, a pre-existing gap surfaced by this PR.** Neither job had ever run `dbt deps`. This went unnoticed until the immediately-prior session's commit (`499344d`, adding a dbt-expectations row-count test) introduced `packages.yml`'s first real dependency — `dbt build` then failed with `Compilation Error: dbt found 1 package(s) specified in packages.yml, but only 0 package(s) installed`. Added a `Run dbt deps` step to both jobs, between `Install dependencies`/`Generate profiles.yml` and `Run dbt build`.

**4. `state:modified+` attempted, then reverted — real gap, not a quick fix.** Tried `dbt build --select state:modified+` for the PR job per the prior session's CI-shape design, but hit `Runtime Error: Got a state selector method, but no comparison manifest` — state-based selection needs a prior `manifest.json` to diff against, and nothing in `ci.yml` produces or fetches one. Reverted the PR job to a plain full `dbt build` to unblock the PR, and moved this from "implemented" back to an explicit on-the-horizon note in CLAUDE.md's CI Architecture Notes, naming two untested candidate approaches (GitHub Actions artifact handoff from the merge job; piggybacking `manifest.json` onto the existing `gh-pages` docs publish for HTTP fetch). Deliberately low-priority at the current model count (4) — full builds are cheap.

**5. CLAUDE.md doc-maintenance pass.** Corrected several "decided, not implemented" bullets left stale by this session's actual implementation work (DuckDB removal, credential injection, dbt deps), and merged genuine duplication that had accumulated across sessions — the Dagster/VPS abandonment narrative was being retold in three separate places, the CI role-scoping follow-up was split across two overlapping bullets, and the "Snowflake job steps" bullet described the now-replaced tempfile credential approach. Brought the file from ~40,888 back down to ~39,403 characters, back under the 40k limit. No cuts to the RBAC/CI/Git gotcha sections — those stay as-is by design.

**6. Branch-protection ruleset was stale, blocking merge.** PR #39 was green but `gh pr merge` failed: `the base branch policy prohibits the merge`. The repo's ruleset (`main protection`, id `17092914`) required a status check named `Load data and run dbt build` — the PR job's name *before* this PR's rename to `Run dbt build against Snowflake DEV`. Confirmed the merge job's check name (`Run dbt build against Snowflake`) was unchanged by this PR and was never itself a required check, so this was the only stale entry — not two. Updated manually via the GitHub UI (Settings → Rules → Rulesets) rather than via `gh api`, since a Claude Code auto-mode classifier blocked the API mutation as a sensitive shared-infrastructure change. PR #39 merged normally afterward, no `--admin` bypass needed.

**Next actions (updated):**
1. Design GitHub Actions-based production scheduling for the `games` dlt pipeline + dbt build (cron trigger, credential handling, run-failure alerting) — still open
2. Design "run succeeded but data is wrong" observability (zero-row pulls, stale/duplicate data) — still open
3. Wire up a comparison `manifest.json` for `state:modified+` in the PR job, once model count makes full builds expensive enough to justify it
...
### 2026-09-04 — `games_pipeline.yml` production scheduling: concurrency fix, freshness-check honesty pass, deprecation cleanup

**Summary: closed out this session's Next Action #1** (GitHub Actions-based production scheduling) with `games_pipeline.yml` — daily cron (6:47am Eastern) + `workflow_dispatch`, running the current-season `games` dlt pipeline then a full `dbt build` against Snowflake `PROD`. A same-session self-review (reviewing `games_pipeline.yml` against `mlb_games.py` and `sources.yml` before committing) surfaced three issues, all fixed on `fix/games-pipeline-concurrency-and-freshness`:

**1. Concurrency race, real risk — fixed.** Nothing stopped a manual `workflow_dispatch` from overlapping the daily cron (or two dispatches back to back), which would have run two concurrent dlt merges and/or `dbt build`s against the same Snowflake `PROD` schema. Added `concurrency: {group: games-pipeline, cancel-in-progress: false}` to the job. `cancel-in-progress` deliberately stays `false` — dlt's SIGTERM handling is for graceful shutdown mid-load, not safe cancellation; killing a run outright could leave a partial merge in an inconsistent state.

**2. Freshness-check comment was making a claim the check can't back up — fixed, not restructured.** The original comment said the `dbt source freshness` step gates against stale data before an expensive build. It doesn't, and can't yet: `games()` does a full re-pull + `merge` every run, so a successful ingestion step always updates every row's `_dlt_load_id` in `_dlt_loads` regardless of whether the MLB API actually returned anything new. The check can only fail if the *previous* run(s) didn't happen — e.g., a missed/silently-disabled cron (GitHub disables scheduled workflows after 60 days of repo inactivity) — not "today's pull found nothing new," since by the time freshness runs, today's ingestion already succeeded. Rewrote the comment to say plainly what the check actually is: a sanity check, not a true staleness gate, that can't distinguish "genuinely new data" from "ran, found nothing new." Left the check itself in place and the step order unchanged — restructuring it to gate on something more meaningful needs a real cursor, which `games()` doesn't have (see below), so this is an honest-documentation fix, not a functional one.

**3. `loaded_at_query` deprecation warning — fixed and verified, not just moved.** `sources.yml` had `loaded_at_query` as a top-level sibling of `config:` on the `games` table, which throws `PropertyMovedToConfigDeprecation` on every dbt invocation (first seen in PR #39's CI logs). `games_pipeline.yml` is the first place this project runs `dbt source freshness` unattended on a schedule, so let this ride until dbt turns the deprecation into a hard error, and a nightly run breaks with nobody watching. Moved `loaded_at_query` to be nested under `config:` alongside `freshness`. Verified rather than assumed: ran `dbt source freshness --target dev --select source:mlb.games` against real Snowflake `DEV` after the move. Confirmed via dbt-core's freshness runner source (`dbt/task/freshness.py`) that `loaded_at_query` still takes precedence over the metadata-cache fallback path regardless of nesting, then confirmed the actual query result — last load `2026-09-02 02:39`, checked at `2026-09-04 17:48`, correctly past the 48-hour `error_after` threshold — so the `ERROR STALE` result reflects genuine data age, not a broken query.

**CLAUDE.md:** added a `games_pipeline.yml` entry to CI Architecture Notes covering the schedule, concurrency guard, and the freshness-check caveat; updated the Orchestration stack-table row and Current Status to reflect production scheduling as implemented rather than a next action; updated "Still open" to the real remaining gap (run-failure alerting, not yet designed). Trimmed elsewhere to stay under the 40k-character limit (39,916 after edits) — no RBAC/CI/Git gotcha content cut.
